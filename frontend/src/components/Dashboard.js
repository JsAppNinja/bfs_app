...
class DashboardComponent extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.props.basicDataLoaded !== prevProps.basicDataLoaded && this.props.basicDataLoaded) {
            this.props.getHomePageData();
            this.props.getConstructionTypeDataHome();
        }
        
        if (this.props.homeData.length !== prevProps.homeData.length && this.props.homeData.length > 0) {
            let videos = [];
            let allvideos = [];
            let dashdata = [];
            let video = {};
    ...
    selectVideo(index) {
        ...
        if (index !== this.state.videoIndex) {
            ...
            this.setState({ videoIndex: index, isPlaying: false, paused: true, pausedonmobile: true })
        } else {
            ...
            titleelement = document.getElementsByClassName("vm-layout");
            titleelement[0].classList.add("d-none");
            this.setState({ videoIndex: index, isPlaying: !this.state.isPlaying, paused: !this.state.paused, pausedonmobile: !this.state.pausedonmobile });
        }
    }
    ...
    */ 
    next() {
        ...
        this.setState({ pausedonmobile: true, videoIndex: nextIndex, paused: true, mobiletitle });
        var imgelement = document.getElementsByClassName("imagepart");
        var videoelement = document.getElementsByClassName("videopart");
        imgelement[0].classList.remove("hideele");
        imgelement[0].classList.add("showele");
    ...
    */ 
    ...
    render() {
        ...
        const slides = mobilevideos.map((item, i) => {
            return (
                <CarouselItem key={i}>
                    <div onMouseEnter={this.showText} onMouseLeave={this.hideText} className="imagepart showele">
                        <img className="imagepart showele" src={item.sliderimage} alt="sliderimage" />
                        {this.state.showLoader ? null : (<div className="video-start" onClick={() => this.selectVideo(this.state.videoIndex, this.state.paused)}></div>)}
                    </div>
                    ...
                    <CarouselCaption className="videocaption " captionText="" captionHeader={item.videoLabel}>
                    </CarouselCaption>
                </CarouselItem>
            );
        });
