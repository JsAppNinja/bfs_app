...
;(function ($, document, window, undefined) {
	...
	$.fn.userTimeout = function (opts) {
        ...
        $(document).ajaxComplete(function (event, xhr, settings) {
            if (settings.url !== "/umbraco/api/keepalive/keepalive") {
                startTimer();
            }
        });
		...
		var modal = function () {
			...
			if ($modalUI === 'bootstrap') {
				...
				if ($options.timer === true) {
                    $footer = $('<div class="modal-footer"><button type="button" class="btn btn-primary" data-dismiss="modal" onclick="callKeepAlive();">' +  $options.modalStayLoggedBtn + ' (<span id="countdowntimer">' + $countDownTimer + '</span>)</button></div>');
				} else {
                    $footer = $('<div class="modal-footer"><button type="button" class="btn btn-primary" data-dismiss="modal" onclick="callKeepAlive();">' +  $options.modalStayLoggedBtn + '</button></div>');
				}
...
var callKeepAlive = function () {
    $.ajax({
        url: '/umbraco/api/keepalive/keepalive'
    });
}